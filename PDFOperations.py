...
def _analyse_steps(file_info: _FileInfo, collection: SpecialCollection) -> str:
    ...
    # if step == 0:
    #     next_value = _step_00_check_entry_result(file_info, collection)
    # elif step == 1:
    #     next_value = _step_01_check_section(file_info, collection)
    # elif step == 2:
    #     next_value = _step_02_check_judging_panel(file_info, collection)
    # elif step == 3:
    #     next_value = _step_03_check_section_data(file_info, collection)
    # elif step == 4:
    #     next_value = _step_04_check_sequenz(file_info, collection)
    # elif step == 5:
    #     next_value = _step_05_check_competition(file_info, collection)
    else:
        # todo change print
        print(fr'Finished')
    return next_value
...
class PDFFile:
    ...
    def read(self):
        ...
        # Set next value to find
        find_next = self.pdf_values.entry_cnt
        # Loop through each page in the PDF
        for page_no, page_layout in enumerate(pdf_pages, start=0):
            # Get the current page from the PDF
            print(fr'Read page {page_no + 1:02d}')
    ...
    def __analyse_steps(self, file_info: __FileInfo) -> str:
        ...
        if step == 0:
            # set next find_str
            next_value = self.pdf_values.judging_panel
            # set next step
            file_info.step += 1
            file_info.clear_page_data()
            # output - for info
            ...
            # Run function
            self.__analyse_clubs(file_info, self.pdf_values.entry_cnt, self.pdf_values.judging_panel)
            # Set max section in competition
            ...
            # set next find_str
            next_value = self.pdf_values.segment
            # set next step
            file_info.step += 1
            file_info.clear_page_data()
            # output - for info
            ...
            self.__analyse_judging_panel(file_info, self.pdf_values.judging_panel, self.pdf_values.segment)
            
            # set next find_str
            next_value = self.pdf_values.competition_sequenz
            # set next step
            file_info.step += 1
            file_info.clear_page_data()
            # output - for info
            ...
            # set next find_str
            next_value = self.pdf_values.heat + ' 1/'
            # set next step
            file_info.step += 1
            file_info.clear_page_data()
            # output - for info
            ...
            file_info.competition_no = self.__analyse_sequenz(file_info, self.pdf_values.competition_sequenz)
            ...
            if file_info.competition_no == -1:
                ...
                file_info.section_no += 1
                # end or go to next
                next_value = self.__end_or_next_section(file_info, self.pdf_values.segment)
            else:
                # Set max competition_no for this segment
    ...
    def __analyse_clubs(self, file_info: __FileInfo, start_value: str, stop_value: str):
        ...
        while index < len(keys) and club_index == -1:
            ...
            for i in range(len(entries)):
                entry = entries[i]
                if entry.text == self.pdf_values.club and not file_info.pages_data[keys[index - 1]][0].text.startswith(
                        'noch'):
                    length = len(entries)
                    association = Association.from_string(file_info.pages_data[keys[index - 1]][0].text)
                    club_index = i
        ...
        while index < len(keys):
            ...
            for index in range(index, len(keys)):
                text_list: list = file_info.pages_data[keys[index]]
                if len(text_list) == length and text_list[club_index].text == self.pdf_values.club:
                    association_str = file_info.pages_data[keys[index - 1]][0].text
    ...
    def __analyse_judging_panel(self, file_info: __FileInfo, start_value: str, stop_value: str):
        ...
        for entry in file_info.pages_data.values():
            if len(entry) > 1:
                if entry[len(entry) - 1].text != self.pdf_values.club:
                    # Add club
                    last = entry[len(entry) - 1]
                    if last.text in club_list:
                        index = club_list.index(last.text)
    ...
    def __analyse_competition(self, file_info: __FileInfo, competition_no: int) -> int:
        ...
        for entry in file_info.pages_data.values():
            ...
            # check for 4 or 5 values and not 4 value is club to identify a lane
            if 4 <= len(entry) <= 5 and entry[3].text != self.pdf_values.club:
                # Check entry length
                if len(entry) == 4:
                    # In case of four club and year a one
                    time_str = entry[3].text
                ...
                # Check if list ot lane
                if not heat and self.pdf_values.lane not in entry[0].text:
                    # list entry
                    lane_str = entry[0].text.replace('.', '').strip()
                    # set list entry
                    list_entry = True
                    ...
                    # list entry
                    lane_str = entry[0].text.replace(self.pdf_values.lane, '').strip()
                    # set list entry
                    list_entry = False
